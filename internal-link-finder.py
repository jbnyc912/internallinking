...
def main():
    ...
    st.text("First, upload the list of URLs you would like to check in a CSV file with the URLs in column A and no header")
    ...
    if uploaded_file is not None:
        ...
        st.text("Paste relevant keywords or terms below, one per line")
        keywords = st.text_area("", placeholder="payday loans\nonline casino\ncbd vape pen", height=150)
        keywords = keywords.split("\n")
        
        # Selector
        st.subheader("HTML Selector")
        st.text("Optional: Enter an HTML selector to narrow down the crawl scope & avoid sitewide elements")
        selector = st.text_input("", placeholder="Enter HTML selector (e.g., .content)")


        # Target URL
        st.subheader("Target URL")
        st.text("Target URL you're looking to add internal links to")
